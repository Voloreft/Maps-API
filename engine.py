...
class PIL(QWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            if 150 <= event.x() <= 749 and 50 <= event.y() <= 499:
                shift_x = (event.x() - 450) * 0.0066
                shift_y = (event.y() - 275) * 0.01
                self.llc = (self.ll[0] + 1.4 ** (-int(self.z)) * shift_x) % 180, self.ll[1]
                if shift_y < 0:
                    self.llc = self.llc[0], min(self.ll[1] - 1.5 ** (-int(self.z)) * shift_y, 89)
                elif shift_y > 0:
                    self.llc = self.llc[0], max(self.ll[1] - 1.5 ** (-int(self.z)) * shift_y, -89)
                self.find_object(stc=True)
                self.update_pic()
